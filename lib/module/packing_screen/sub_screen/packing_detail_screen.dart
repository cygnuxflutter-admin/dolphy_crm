...
import '../model/vendor_responce_model.dart';
...
class PackingDetailScreen extends GetView<PackingController> {
        ...
        actions: [
          Obx(() {
            ...
            return PopupMenuButton<String>(
              ...
              onSelected: (value) {
                if (value == 'reject') {
                  if (data.status?.toUpperCase() == "DRAFT") {
                    _showCancelPackingDialog(context, data.id!);
                  } else {
              ...
              itemBuilder: (context) => [
                if (data.status != "REJECTED" && data.status != "INVOICED")
                  PopupMenuItem(
                    value: 'reject',
                    child: Row(
                      children: [
                        Icon(data.status?.toUpperCase() == "DRAFT" ? Icons.cancel_outlined : Icons.block, color: AppColors.red500, size: 20),
                        const SizedBox(width: 8),
                        Text(data.status?.toUpperCase() == "DRAFT" ? "Cancel" : "Reject", style: const TextStyle(color: AppColors.red500)),
                      ],
                    ),
                  ),
              ],
                ...
                child: Row(
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: controller.detailStatusColor.value.withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(100),
                        border: Border.all(color: controller.detailStatusColor.value.withValues(alpha: 0.3), width: 0.5),
                      ),
                      child: Text(
                        controller.detailStatusLabel.value.toUpperCase(),
                        style: TextStyle(color: controller.detailStatusColor.value, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5),
                      ),
                    ),
                  ],
                ),
            ...
            child: Column(
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: _infoCell(Icons.account_balance_wallet_outlined, "Taxable Amount", "₹${data.invoice?.totalTaxableAmount ?? "0.00"}"),
                    ),
                      ...
                      children: [
                        ...
                        Obx(
                          () => ElevatedButton.icon(
                            ...
                            label: Text(
                              controller.isLoading.value ? "Confirming..." : "Confirm",
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: AppColors.white,
              border: Border(bottom: BorderSide(color: AppColors.gray200.withValues(alpha: 0.5), width: 0.8)),
            ),
            child: const Row(
              children: [
                Icon(Icons.list, size: 18, color: AppColors.textSecondary),
                SizedBox(width: 8),
                Text(
                  "Items",
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      _tableHeaderCell("Product Code", width: 120),
                      _tableHeaderCell("Product Name", width: 180),
                      _tableHeaderCell("Batch Qty", width: 100),
                      _tableHeaderCell("Batch Numbers", width: 150),
                      _tableHeaderCell("Picked Qty", width: 100),
                      _tableHeaderCell("Packed Qty", width: 100),
                    ],
                  ),
                ),
                Container(
  ...
  Widget _tableHeaderCell(String text, {required double width}) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(right: BorderSide(color: AppColors.gray200.withValues(alpha: 0.5), width: 0.8)),
      ),
      child: Text(
        text,
